...
int calculateLEDBrightness(float distance) {
  if (distance <= 100 || distance >= 300) {
    ...
    return map(distance, 200, 300, 0, 255); // 밝음 → 어두움
  } else if (distance >= 100 && distance < 200) {
    return map(distance, 100, 200, 255, 0); // 어두움 → 밝음
  } else {
    return 127; // 안전값
  }
}
...
void loop() { 
  ...
  if ((distance == 0.0) || (distance < _DIST_MIN) || (distance > _DIST_MAX)) {
    analogWrite(PIN_LED, 255); // 꺼짐 (active low → 255가 최소 밝기)
} else {
    int brightness;
    
    // 거리 기반 비례 밝기 조절
    if (distance <= 200) {
      brightness = map(distance, 100, 200, 255, 0); // 가까우면 어두움 → 밝게
    } else {
      brightness = map(distance, 200, 300, 0, 255); // 멀어지면 밝게 → 어둡게
    }
